...
class DbServer {
    ...
    listen(callback){
        ...
        this._db.createServer(function(port, host){
            ...
            // init the http and ws instances
    ...
    init(callback){
        let self = this
        this._serverHttp = new ServerHttp(this._port, this._host, this._db, this._pubsub, this.options)
        // call middlewares
        this._serverHttp.use(this.middleware._http)
        this._serverHttp.useBefore(this.middlewareBefore._http)
        //server listen
    ...
    intWs(){
        ...
        this._serverWs = new ServerWs(this._serverHttp.server, this._db, this._pubsub, this.options)
        //call middlewares
        this._serverWs.use(this.middleware._ws)
        this._serverWs.useBefore(this.middlewareBefore._ws)
    }
