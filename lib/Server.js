...
class DbServer extends EventEmitter {
        ...
        this._wss.on('connection', function connection(ws, req) {
            ws.on('message', function incoming(data) {
                try {
                    ...
                    console.error(e)
                    ws.send(JSON.stringify({ err : 'internal error while parsing message', data : null}))
                }
            })
    ...
    */
    ...
    /**
     * Database handler
     * @param {string} src source where the data comes from _ws or _http
     * @param {object} data 
     * @param {function} callback 
     */
