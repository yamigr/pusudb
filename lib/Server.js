...
class DbServer extends EventEmitter {
    ...
    listen(callback){
        ...
        this._db.createServer(function(port, host){
            ...
            self._serverHttp = new ServerHttp(self._port, self._host, self._db, self._pubsub, self.middleware._http, self.options)
            ...
            self._serverWs = new ServerWs(self._serverHttp.server, self._db, self._pubsub, self.middleware._ws, self.options)
        })
    }
