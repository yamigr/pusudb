...
const debug = require('debug')('pusudb:app')
...
class DbServer {
    ...
    constructor(port, host, opt){
        ...
        this.options.heartbeat = this.options.heartbeat || 30000
    ...
    listen(callback){
        ...
        debug('booting %o', this.name)
        debug('options %o', JSON.parse(JSON.stringify(this.options, null, 2)))

        this.db.listen(function(port, host){

            debug('database-server listening', port, host)
