            ...
            async.series([
                function(next){
                    try{
                        ...
                        req.body = Object.assign( req.body, self.parseParamInBody(req.body))
                        next()
                    }catch(e){
                        next(e.toString())
                    }
                ...
                function(next){
                    // Database
                    if(self.hasPrefix(req.url) && !self.options.db_list.length || self.hasPrefix(req.url) && self.options.db_list.indexOf(req.body.noExtDb) !== -1) {
                        self.handlerQueryAndPubSub(req.body, ws, function(docs){
                            req.docs = Object.assign(req.docs, { db : req.body.noExtDb, meta : req.body.meta}, docs)
                            next()
                        })
                    }
                    else{
