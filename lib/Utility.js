    ...
    handlerQueryAndPubSub(data, req, ws, callback){
        ...
        if(!data.db || !data.data){
            ...
            if(data.meta === 'subscribe' || data.meta === 'unsubscribe' || data.meta === 'publish'){
                // Log to console
                this.pubsub.handle( data.db, data.meta, data.data, req, ws)
                    ...
                    this.db.query( helpers.concatDbPath(this.options.path, data.db) , data.meta, data.data, function(docs){
                        ...
                        if(data.meta === 'update'){
                            self.db.query( helpers.concatDbPath(self.options.path, data.db)  , 'get', { key : docs.data }, function(docs){
                                self.pubsub.handle( data.db, data.meta, docs.data, req, ws )
                            }) 
                        }
                        else{
                            self.pubsub.handle( data.db, data.meta, data.data, req, ws)
                        }
