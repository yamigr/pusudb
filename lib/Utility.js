...
class Utility extends EventEmitter {
    ...
    enableQueryDb(url, api){
        return this.hasPrefix(url) && 
               (!this.options.db_list.length && !this.options.db_block.length) || 
               (this.options.db_list.indexOf(api.noExtDb) !== -1 && this.options.db_block.indexOf(api.noExtDb) === -1)
    }
    ...
    handlerQueryAndPubSub(data, ws, callback){
        ...
        if(!data.db || !data.data){
            ...
            if(data.meta === 'subscribe' || data.meta === 'unsubscribe' || data.meta === 'publish'){
                // Log to console
                this.pubsub.handle( data.noExtDb, data.meta, data.data, ws)
                ...
                if(data.meta === 'list'){
                    ...
                    this.db.query(data.db, data.meta, data.data, function(docs){
                        ...
                        self.pubsub.handle( data.noExtDb, data.meta, data.data, ws)
                        callback(docs)
                    }) 
                }
            }
