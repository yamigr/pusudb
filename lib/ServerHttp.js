    ...
    */
    ...
    handelQuery(req, callback){
        ...
        switch(req.method){
            ...
            if(req.pusudb.meta === 'list'){
                this.listDb(this.parseList(req.params.query, ','), function(docs){
                    callback(docs)
                })
            }
            else{
            ...
            if(req.pusudb.meta === 'list'){
                this.listDb(req.body, function(docs){
                    callback(docs)
                })
            }
            else{
    ...
    /**
     * Parses the query string to object /select/list?one=db,stream,limit 5,reverse true&two=db,get,key @mykey
