...
class HttpServer {
    ...
    */
    ...
    middleware(req, res){
        ...
        async.parallel([
            ...
            function(done){
                req.docs = { err: '', data : ''}
                if(req.pusudb.db && req.pusudb.meta){
    ...
    */
    ...
    handelQuery(req, callback){
        ...
        switch(req.method){
            ...
            if(req.pusudb.meta === 'list'){
                this.queryList(this.parseList(req.params.query, ','), function(docs){
                    callback(docs)
                })
            }
            else{
            ...
            if(req.pusudb.meta === 'list'){
                this.queryList(req.body, function(docs){
                    callback(docs)
                })
            }
            else{
    ...
    parseParam(path){
        ...
        if(!path.startsWith(prefix) && (!obj.path || obj.path === '/')){
            obj.path = '/index'
        }
        try{
