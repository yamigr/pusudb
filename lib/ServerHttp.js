...
class HttpServer {
    ...
    */
     ...
     * @param {string} dbname databasename with given path
     * @param {string} meta 
    ...
    handler(dbname, meta, data, callback){
        ...
        if(data){
            if(meta === 'publish'){
                self.pubsub.handle( dbname, self.pubsub.convertMeta(meta) , data, null)
                callback()
            }
            else{
                // Query the database
                this.db.query(dbname, meta, data, function(docs){
                    ...
                    if(meta === 'batch'){
                        data = self.pubsub.convertBatch(data)
                    }
