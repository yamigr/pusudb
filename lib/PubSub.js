...
const debug = require('debug')('pusudb:pubsub')
...
class PubSub {
     ...
     * @param {object} sock 
     */
    handle(noExtDb, meta, data, req, sock){
        debug('db: %o, meta: %o', noExtDb, meta)
        switch(meta){
            case 'del':
                // publish delete and delete all sockets
                this.publish(noExtDb, meta, data.data.key, this.getWsKey(req))
                if(Object.keys(this.wildcards).length){
                    this.wildcardpublish(noExtDb, meta, data.data.key,  this.getWsKey(req))
                }
                this.unsubscribeAll(noExtDb + data.data.key,  this.getWsKey(req))  
            break
            case 'put':
            case 'batch':
            case 'update':
            case 'publish':
            case '___custom':
                this.publish(noExtDb, meta, data.data,  this.getWsKey(req))
                if(Object.keys(this.wildcards).length){
                    this.wildcardpublish(noExtDb, meta, data.data,  this.getWsKey(req))
                }
            break
            case 'subscribe':
                ...
                if(typeof data.data === 'string'){
                    this.subscribe(noExtDb + data.data, this.getWsKey(req), sock)  
                }
                else{
                    for(let t in data.data){
                        this.subscribe(noExtDb + data.data[t], this.getWsKey(req), sock)         
                    }
                }
            break
            case 'unsubscribe':
                ...
                if(typeof data.data === 'string'){
                    this.unsubscribe(noExtDb + data.data, this.getWsKey(req) )  
                }
                else{
                    for(let t in data.data){
                        this.unsubscribe(noExtDb + data.data[t], this.getWsKey(req) )         
                    }
                }
            break
            case 'destroy':
